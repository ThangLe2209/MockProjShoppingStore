...
namespace ShoppingStore.Client.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public ProductController(ShoppingStoreService shoppingStoreService, IWebHostEnvironment webHostEnvironment)
        {
        ...
        {
            return View(await _shoppingStoreService.GetProductsAsync(""));
        }

        [HttpGet]
        public async Task<IActionResult> Create()
        {
            ViewBag.Categories = new SelectList(await _shoppingStoreService.GetCategoriesAsync(), "Id", "Name");
            ViewBag.Brands = new SelectList(await _shoppingStoreService.GetBrandsAsync(), "Id", "Name");
            return View();
        }
        ...
        {
            ViewBag.Categories = new SelectList(await _shoppingStoreService.GetCategoriesAsync(), "Id", "Name", product.CategoryId); // Compare product.CategoryId with CategoryId in Categories context and return ID,Name
            ViewBag.Brands = new SelectList(await _shoppingStoreService.GetBrandsAsync(), "Id", "Name", product.BrandId);
            ...
            if (ModelState.IsValid)
            {
                ...
                using (var multipartFormContent = new MultipartFormDataContent())
                {
                    ...
                    using var response = await _shoppingStoreService.CreateProductAsync(multipartFormContent);
        ...
        public async Task<IActionResult> Edit(Guid Id)
        {
            ...
            ViewBag.Categories = new SelectList(await _shoppingStoreService.GetCategoriesAsync(), "Id", "Name", productEdit.CategoryId); // Compare product.CategoryId with CategoryId in Categories context and return ID,Name
            ViewBag.Brands = new SelectList(await _shoppingStoreService.GetBrandsAsync(), "Id", "Name", productEdit.BrandId);
        ...
        public async Task<IActionResult> Edit(Guid Id, ProductForEditDto product)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (var multipartFormContent = new MultipartFormDataContent())
                {
                    ...
                    using var response = await _shoppingStoreService.UpdateProductAsync(Id, multipartFormContent);
                    //response.EnsureSuccessStatusCode();
                    if (response.IsSuccessStatusCode) // can use httpResponseMessage.EnsureSuccessStatusCode(); to treat statusCode as error data response
                    {
                        //var responseContent = await response.Content.ReadAsStringAsync();
        ...
        {
            using HttpResponseMessage response = await _shoppingStoreService.DeleteProductAsync(Id);
            if (response.IsSuccessStatusCode) // can use httpResponseMessage.EnsureSuccessStatusCode(); to treat statusCode as error data response
            {
                //var responseContent = await response.Content.ReadAsStringAsync();
                //Console.WriteLine("UploadFileInModel_WithHttpClientAsync response :" + responseContent);
