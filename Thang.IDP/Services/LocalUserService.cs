...
namespace Thang.IDP.Services
{
    public class LocalUserService : ILocalUserService
    {
        ...
        public User AutoProvisionUser(string provider, string providerIdentityKey, IEnumerable<Claim> claims) // create new User to db when user login with External Provider but don't have data in UserLogin table
        {
            if (string.IsNullOrWhiteSpace(provider))
            {
                throw new ArgumentNullException(nameof(provider));
